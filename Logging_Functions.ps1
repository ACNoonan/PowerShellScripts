Function Log-Start{
  ...
  Process{
        #check if the directory exits.  Create if not
    If(!(Test-Path -Path $LogPath))
    {
        New-Item -ItemType directory -Path $LogPath
    }
    
    #insert the timestamp in the filename
    $dt = Get-Date -Format yyyyMMdd_hhmmss_fff
    $LogName = [System.IO.Path]::GetFileNameWithoutExtension($LogName) + "_" + $dt + [System.IO.Path]::GetExtension($LogName)
